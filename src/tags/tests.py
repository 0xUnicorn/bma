...
class TestTagsApi(ApiTestBase):
    ...
    def test_tag_api(self) -> None:
        ...
        for i in range(5):
            ...
            response_tags = [{"name": tag, "weight": 1} for tag in tags]
        ...
        for i in range(2, 7):
            ...
            response_tags = [{"name": "bar", "weight": 1}]
            if i <= 4:
                # these files are also tagged foo
                response_tags.append({"name": "foo", "weight": 1})
                # weight is now 2 for tagN for these files
                response_tags.insert(0, {"name": f"tag{i}", "weight": 2})
            else:
                response_tags.append({"name": f"tag{i}", "weight": 1})
            assert response.json()["bma_response"] == response_tags
        ...
        for i in range(3):
            ...
            if i > 1:
                response_tags = [{"name": f"tag{i}", "weight": 2}, {"name": "bar", "weight": 1}]
            else:
                response_tags = [{"name": f"tag{i}", "weight": 1}]
